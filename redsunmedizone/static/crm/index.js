...
$(function($) {
	...
	$('#customer_list_toolbar').find('div[data=search]').searchbox({
...
function addUser(){
	...
	$.messager.confirm('添加操作', '确定要添加用户吗?!(5个选项要全部选到才可以!)', function(r){
		if (r){
	
	var data = {}
	var sort = $('div[data=sort]').slider('getValue')
	...
	$('div[render=value]').each(function(){
		var key = $(this).attr('data')
		var value = $(this).textbox('getText')
		data[key] = value
	})
...
function customerDetail(id){
	...
	$.ajax({url:'/customer_detail/',type:'POST',data:{'id':id},dataType:"json",success:function(data){
		...
		$('div[data=sort]').slider('setValue',data.sort)
...
function saveUser(){
	...
	$.messager.confirm('保存操作', '确定要保存用户吗?!(需要选中表格中任意一行!并且5个选项要全部选到才可以!)', function(r){
		if (r){
